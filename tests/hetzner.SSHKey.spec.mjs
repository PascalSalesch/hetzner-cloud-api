...
import hetzner, { SSHKey } from '../api.mjs'
...
describe('hetzner.SSHKey', async () => {
  it('AsyncIterator should respond with SSHKey instances', async () => {
    for await (const sshKey of hetzner.SSHKey) {
      assert.instanceOf(sshKey, SSHKey)
    }
  })
})
